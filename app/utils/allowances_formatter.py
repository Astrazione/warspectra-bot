...
class AllowancesFormatter:
    ...
    def get_allowances_str(player_info: Dict[str, str]):
        displayed_allowances = ["pCYP", "pBTV", "pRP", "pKMB", "pSkill"]
    ...
    def get_allowance_type_str(allowance_type: str, player_info: Dict[str, str]) -> str:
        ...
        allowance_names = ", ".join([allowance_field.items[allowance_index].name for allowance_index in range(len(allowances)) if allowances[allowance_index] == "1"]) # type: ignore
